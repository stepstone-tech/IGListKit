...
@protocol IGSTListUpdatingDelegate;

@class IGSTListSectionController;
...
typedef void (^IGSTListUpdaterCompletion)(BOOL finished);

/**
 `IGSTListAdapter` objects provide an abstraction for feeds of objects in a `UICollectionView` by breaking each object 
 into individual sections, called "section controllers". These controllers (objects subclassing to 
 `IGSTListSectionController`) act as a data source and delegate for each section.

 Feed implementations must act as the data source for an `IGSTListAdapter` in order to drive the objects and section
 controllers in a collection view.
 */
IGLK_SUBCLASSING_RESTRICTED
NS_SWIFT_NAME(ListAdapter)
@interface IGSTListAdapter : NSObject
 ...
 */
@property (nonatomic, nullable, weak) id <IGSTListAdapterDataSource> dataSource;

/**
 The object that receives top-level events for section controllers.
 */
@property (nonatomic, nullable, weak) id <IGSTListAdapterDelegate> delegate;
 ...
 */
@property (nonatomic, strong, readonly) id <IGSTListUpdatingDelegate> updater;

/**
 A bitmask of experiments to conduct on the adapter.
 */
@property (nonatomic, assign) IGSTListExperiment experiments;

/**
 Initializes a new `IGSTListAdapter` object.
...
/**
 Initializes a new `IGSTListAdapter` object with a working range of `0`.
 ...
 */
- (void)performUpdatesAnimated:(BOOL)animated completion:(nullable IGSTListUpdaterCompletion)completion;
...
/**
 ...
 @see `-[IGSTListAdapterDataSource listAdapter:sectionControllerForObject:]`
 */
- (__kindof IGSTListSectionController * _Nullable)sectionControllerForObject:(id)object;
 ...
 */
- (nullable id)objectForSectionController:(IGSTListSectionController *)sectionController;
