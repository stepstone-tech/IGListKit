...
@implementation IGSTListAdapter {
    NSMapTable<UICollectionReusableView *, IGSTListSectionController *> *_viewSectionControllerMap;
}
...
- (instancetype)initWithUpdater:(id <IGSTListUpdatingDelegate>)updater
    ...
    if (self = [super init]) {
        ...
        _sectionMap = [[IGSTListSectionMap alloc] initWithMapTable:table];

        _displayHandler = [IGSTListDisplayHandler new];
        _workingRangeHandler = [[IGSTListWorkingRangeHandler alloc] initWithWorkingRangeSize:workingRangeSize];
        ...
        [IGSTListDebugger trackAdapter:self];
    }
    return self;
}
...
- (void)setCollectionView:(UICollectionView *)collectionView {
    ...
    if (_collectionView != collectionView || _collectionView.dataSource != self) {
        // if the collection view was being used with another IGSTListAdapter (e.g. cell reuse)
        // destroy the previous association so the old adapter doesn't update the wrong collection view
        static NSMapTable<UICollectionView *, IGSTListAdapter *> *globalCollectionViewAdapterMap = nil;
...
- (void)setDataSource:(id<IGSTListAdapterDataSource>)dataSource {
...
- (void)setCollectionViewDelegate:(id<UICollectionViewDelegate>)collectionViewDelegate {
    ...
    IGAssert(![collectionViewDelegate conformsToProtocol:@protocol(UICollectionViewDelegateFlowLayout)],
             @"UICollectionViewDelegateFlowLayout conformance is automatically handled by IGSTListAdapter.");
...
- (void)updateAfterPublicSettingsChange {
    id<IGSTListAdapterDataSource> dataSource = _dataSource;
...
- (void)createProxyAndUpdateCollectionViewDelegate {
    ...
    self.delegateProxy = [[IGSTListAdapterProxy alloc] initWithCollectionViewTarget:_collectionViewDelegate
...
- (void)performUpdatesAnimated:(BOOL)animated completion:(IGSTListUpdaterCompletion)completion {
    IGAssertMainThread();

    id<IGSTListAdapterDataSource> dataSource = self.dataSource;
    UICollectionView *collectionView = self.collectionView;
...
- (void)reloadDataWithCompletion:(nullable IGSTListUpdaterCompletion)completion {
    IGAssertMainThread();
...
- (void)reloadObjects:(NSArray *)objects {
    ...
    IGSTListSectionMap *map = [self sectionMapUsingPreviousIfInUpdateBlock:YES];
...
- (nullable IGSTListSectionController *)sectionControllerForSection:(NSInteger)section {
    IGAssertMainThread();
    
    return [self.sectionMap sectionControllerForSection:section];
}

- (NSInteger)sectionForSectionController:(IGSTListSectionController *)sectionController {
    IGAssertMainThread();
...
- (IGSTListSectionController *)sectionControllerForObject:(id)object {
    IGAssertMainThread();
    IGParameterAssert(object != nil);
...
- (id)objectForSectionController:(IGSTListSectionController *)sectionController {
    IGAssertMainThread();
    IGParameterAssert(sectionController != nil);
...
- (id<IGSTListSupplementaryViewSource>)supplementaryViewSourceAtIndexPath:(NSIndexPath *)indexPath {
    IGSTListSectionController *sectionController = [self sectionControllerForSection:indexPath.section];
    return [sectionController supplementaryViewSource];
}

- (NSArray<IGSTListSectionController *> *)visibleSectionControllers {
    IGAssertMainThread();
    ...
    for (UICollectionViewLayoutAttributes* attribute in attributes) {
        IGSTListSectionController *sectionController = [self sectionControllerForSection:attribute.indexPath.section];
...
- (NSArray *)visibleObjects {
    ...
    for (UICollectionViewCell *cell in visibleCells) {
        IGSTListSectionController *sectionController = [self sectionControllerForView:cell];
...
- (CGSize)sizeForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    IGAssertMainThread();
    id <IGSTListSupplementaryViewSource> supplementaryViewSource = [self supplementaryViewSourceAtIndexPath:indexPath];
...
- (void)updateObjects:(NSArray *)objects dataSource:(id<IGSTListAdapterDataSource>)dataSource {
    ...
    NSMutableArray<IGSTListSectionController *> *sectionControllers = [NSMutableArray new];
    NSMutableArray *validObjects = [NSMutableArray new];

    IGSTListSectionMap *map = self.sectionMap;
    ...
    // for IGSTListSectionController subclasses after calling [super init]
    IGSTListSectionControllerPushThread(self.viewController, self);

    for (id object in objects) {
        // infra checks to see if a controller exists
        IGSTListSectionController *sectionController = [map sectionControllerForObject:object];
    ...
    IGSTListSectionControllerPopThread();
...
- (BOOL)itemCountIsZero {
    ...
    [self.sectionMap enumerateUsingBlock:^(id  _Nonnull object, IGSTListSectionController * _Nonnull sectionController, NSInteger section, BOOL * _Nonnull stop) {
...
- (IGSTListSectionMap *)sectionMapUsingPreviousIfInUpdateBlock:(BOOL)usePreviousMapIfInUpdateBlock {
    // if we are inside an update block, we may have to use the /previous/ item map for some operations
    IGSTListSectionMap *previousSectionMap = self.previousSectionMap;
...
- (NSArray<NSIndexPath *> *)indexPathsFromSectionController:(IGSTListSectionController *)sectionController
                                                    indexes:(NSIndexSet *)indexes
                                 usePreviousIfInUpdateBlock:(BOOL)usePreviousIfInUpdateBlock {
    NSMutableArray<NSIndexPath *> *indexPaths = [NSMutableArray new];

    IGSTListSectionMap *map = [self sectionMapUsingPreviousIfInUpdateBlock:usePreviousIfInUpdateBlock];
...
- (NSIndexPath *)indexPathForSectionController:(IGSTListSectionController *)controller
...
- (void)mapView:(UICollectionReusableView *)view toSectionController:(IGSTListSectionController *)sectionController {
    IGAssertMainThread();
...
- (nullable IGSTListSectionController *)sectionControllerForView:(UICollectionReusableView *)view {
    IGAssertMainThread();
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    NSArray<IGSTListSectionController *> *visibleSectionControllers = [self visibleSectionControllers];
    for (IGSTListSectionController *sectionController in visibleSectionControllers) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSArray<IGSTListSectionController *> *visibleSectionControllers = [self visibleSectionControllers];
    for (IGSTListSectionController *sectionController in visibleSectionControllers) {
        id<IGSTListScrollDelegate> scrollDelegate = [sectionController scrollDelegate];
...
- (NSInteger)indexForCell:(UICollectionViewCell *)cell sectionController:(nonnull IGSTListSectionController *)sectionController {
    IGAssertMainThread();
...
- (__kindof UICollectionViewCell *)dequeueReusableCellOfClass:(Class)cellClass
    ...
    NSString *identifier = IGSTListReusableViewIdentifier(cellClass, nil, nil);
    NSIndexPath *indexPath = [self indexPathForSectionController:sectionController index:index usePreviousIfInUpdateBlock:NO];
...
- (__kindof UICollectionReusableView *)dequeueReusableSupplementaryViewOfKind:(NSString *)elementKind
    ...
    NSString *identifier = IGSTListReusableViewIdentifier(viewClass, nil, elementKind);
    NSIndexPath *indexPath = [self indexPathForSectionController:sectionController index:index usePreviousIfInUpdateBlock:NO];
...
- (void)performBatchAnimated:(BOOL)animated updates:(void (^)(id<IGSTListBatchContext>))updates completion:(void (^)(BOOL))completion {
    ...
    [self.updater performUpdateWithCollectionView:collectionView animated:animated itemUpdates:^{
        ...
        // the adapter acts as the batch context with its API stripped to just the IGSTListBatchContext protocol
        updates(weakSelf);
...
- (void)invalidateLayoutForSectionController:(IGSTListSectionController *)sectionController
...
- (void)reloadInSectionController:(IGSTListSectionController *)sectionController atIndexes:(NSIndexSet *)indexes {
    ...
    /**
     ...
     IGSTListAdapter tracks the before/after mapping of section controllers to make precise NSIndexPath conversions.
     */
