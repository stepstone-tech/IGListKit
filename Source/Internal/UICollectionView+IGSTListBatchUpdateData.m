...
@implementation UICollectionView (IGSTListBatchUpdateData)

- (void)igst_applyBatchUpdateData:(IGSTListBatchUpdateData *)updateData {
    [self deleteItemsAtIndexPaths:updateData.deleteIndexPaths];
    [self insertItemsAtIndexPaths:updateData.insertIndexPaths];

    for (IGSTListMoveIndexPath *move in updateData.moveIndexPaths) {
        [self moveItemAtIndexPath:move.from toIndexPath:move.to];
    }

    for (IGSTListMoveIndex *move in updateData.moveSections) {
