...
#import "IGSTListBindingSectionController+DebugDescription.h"

#import "IGSTListDebuggingUtilities.h"

@implementation IGSTListBindingSectionController (DebugDescription)
...
- (NSArray<NSString *> *)debugDescriptionLines {
    ...
    for (id<IGSTListDiffable> viewModel in self.viewModels) {
    ...
    [debug addObject:[NSString stringWithFormat:@"Is first section: %@", IGSTListDebugBOOL(self.isFirstSection)]];
    [debug addObject:[NSString stringWithFormat:@"Is last section: %@", IGSTListDebugBOOL(self.isLastSection)]];
