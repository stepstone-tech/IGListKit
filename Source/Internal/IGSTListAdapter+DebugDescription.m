...
#import "IGSTListSectionMap+DebugDescription.h"
...
@implementation IGSTListAdapter (DebugDescription)

- (NSString *)debugDescription {
    NSMutableArray *lines = [NSMutableArray arrayWithObject:[NSString stringWithFormat:@"IGListAdapter %p:", self]];
    [lines addObjectsFromArray:IGSTListDebugIndentedLines([self debugDescriptionLines])];
...
- (NSArray<NSString *> *)debugDescriptionLines {
    ...
    [debug addObject:[NSString stringWithFormat:@"Is in update block: %@", IGSTListDebugBOOL(self.isInUpdateBlock)]];
    [debug addObject:[NSString stringWithFormat:@"View controller: %@", self.viewController]];
    [debug addObject:[NSString stringWithFormat:@"Is prefetching enabled: %@", IGSTListDebugBOOL(self.collectionView.isPrefetchingEnabled)]];
    ...
    if ([self.updater isKindOfClass:[IGSTListAdapterUpdater class]]) {
        [debug addObject:[NSString stringWithFormat:@"IGListAdapterUpdater instance %p:", self.updater]];
        [debug addObjectsFromArray:IGSTListDebugIndentedLines([(IGSTListAdapterUpdater *)self.updater debugDescriptionLines])];
    }

    [debug addObject:[NSString stringWithFormat:@"Section map details:"]];
    [debug addObjectsFromArray:IGSTListDebugIndentedLines([self.sectionMap debugDescriptionLines])];

    if (self.previousSectionMap != nil) {
        [debug addObject:[NSString stringWithFormat:@"Previous section map details:"]];
        [debug addObjectsFromArray:IGSTListDebugIndentedLines([self.previousSectionMap debugDescriptionLines])];
    }

    [debug addObject:[NSString stringWithFormat:@"Collection view details:"]];
    [debug addObjectsFromArray:IGSTListDebugIndentedLines([self.collectionView debugDescriptionLines])];
