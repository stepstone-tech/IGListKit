...
typedef NS_OPTIONS (NSInteger, IGSTListExperiment) {
    /// Specifies no experiments.
    IGSTListExperimentNone = 1 << 1,
    /// Test updater diffing performed on a background queue.
    IGSTListExperimentBackgroundDiffing = 1 << 2,
    /// Test fallback to reloadData when "too many" update operations.
    IGSTListExperimentReloadDataFallback = 1 << 3,
};
...
static inline BOOL IGSTListExperimentEnabled(IGSTListExperiment mask, IGSTListExperiment option) {
...
FOUNDATION_EXTERN IGSTListIndexSetResult *IGSTListDiffExperiment(NSArray<id<IGSTListDiffable>> *_Nullable oldArray,
...
/**
 ...
 @see `IGSTListDiffPaths()`.
 */
NS_SWIFT_NAME(ListDiffPathsExperiment(fromSection:toSection:oldArray:newArray:option:experiments:))
FOUNDATION_EXTERN IGSTListIndexPathResult *IGSTListDiffPathsExperiment(NSInteger fromSection,
                                                                   NSInteger toSection,
