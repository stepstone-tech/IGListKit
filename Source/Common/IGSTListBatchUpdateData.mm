...
static void convertMoveToDeleteAndInsert(NSMutableSet<IGSTListMoveIndex *> *moves,
                                         IGSTListMoveIndex *move,
...
@implementation IGSTListBatchUpdateData

// Converts all section moves that have index path operations into a section delete + insert.
+ (void)cleanIndexPathsWithMap:(const std::unordered_map<NSInteger, IGSTListMoveIndex*> &)map
                         moves:(NSMutableSet<IGSTListMoveIndex *> *)moves
...
- (instancetype)initWithInsertSections:(NSIndexSet *)insertSections
                        deleteSections:(NSIndexSet *)deleteSections
                          moveSections:(NSSet<IGSTListMoveIndex *> *)moveSections
                      insertIndexPaths:(NSArray<NSIndexPath *> *)insertIndexPaths
                      deleteIndexPaths:(NSArray<NSIndexPath *> *)deleteIndexPaths
                        moveIndexPaths:(NSArray<IGSTListMoveIndexPath *> *)moveIndexPaths {
    ...
    if (self = [super init]) {
        NSMutableSet<IGSTListMoveIndex *> *mMoveSections = [moveSections mutableCopy];
        NSMutableIndexSet *mDeleteSections = [deleteSections mutableCopy];
        NSMutableIndexSet *mInsertSections = [insertSections mutableCopy];
        NSMutableSet<IGSTListMoveIndexPath *> *mMoveIndexPaths = [moveIndexPaths mutableCopy];
        ...
        std::unordered_map<NSInteger, IGSTListMoveIndex*> fromMap(moveCount);
        std::unordered_map<NSInteger, IGSTListMoveIndex*> toMap(moveCount);
        for (IGSTListMoveIndex *move in moveSections) {
        ...
        [IGSTListBatchUpdateData cleanIndexPathsWithMap:fromMap moves:mMoveSections indexPaths:mDeleteIndexPaths deletes:mDeleteSections inserts:mInsertSections];

        // prevents a bug where UICollectionView corrupts the heap memory when inserting into a section that is moved
        [IGSTListBatchUpdateData cleanIndexPathsWithMap:toMap moves:mMoveSections indexPaths:mInsertIndexPaths deletes:mDeleteSections inserts:mInsertSections];

        for (IGSTListMoveIndexPath *move in moveIndexPaths) {
            ...
            if (it != fromMap.end() && it->second != nil) {
                IGSTListMoveIndex *sectionMove = it->second;
                [mMoveIndexPaths removeObject:move];
