...
- (instancetype)initWithInserts:(NSIndexSet *)inserts
                        ...
                        updates:(NSIndexSet *)updates
                          moves:(NSArray<IGSTListMoveIndex *> *)moves
...
- (IGSTListIndexSetResult *)resultForBatchUpdates {
    ...
    NSArray<IGSTListMoveIndex *> *moves = self.moves;
    NSMutableArray<IGSTListMoveIndex *> *filteredMoves = [moves mutableCopy];

    // convert all update+move to delete+insert
    const NSInteger moveCount = moves.count;
    for (NSInteger i = moveCount - 1; i >= 0; i--) {
        IGSTListMoveIndex *move = moves[i];
    ...
    return [[IGSTListIndexSetResult alloc] initWithInserts:inserts
                                                 deletes:deletes
