...
/**
 An `IGSTListAdapterUpdater` is a concrete type that conforms to `IGSTListUpdatingDelegate`.
 ...
 for calculating UI updates when `IGSTListAdapter` calls 
 `-performUpdateWithCollectionView:fromObjects:toObjects:completion:`.
 */
IGLK_SUBCLASSING_RESTRICTED
NS_SWIFT_NAME(ListAdapterUpdater)
@interface IGSTListAdapterUpdater : NSObject <IGSTListUpdatingDelegate>

/**
 The delegate that receives events with data on the performance of a transition.
 */
@property (nonatomic, weak) id<IGSTListAdapterUpdaterDelegate> delegate;
