...
#import "UICollectionView+IGSTListBatchUpdateData.h"
#import "IGSTListIndexSetResultInternal.h"
#import "IGSTListMoveIndexPathInternal.h"
#import "IGSTListReloadIndexPath.h"

@implementation IGSTListAdapterUpdater
...
- (instancetype)init {
    ...
    if (self = [super init]) {
        ...
        _batchUpdates = [IGSTListBatchUpdates new];
...
- (void)performReloadDataWithCollectionView:(UICollectionView *)collectionView {
    ...
    id<IGSTListAdapterUpdaterDelegate> delegate = self.delegate;
    void (^reloadUpdates)() = self.reloadUpdates;
    IGSTListBatchUpdates *batchUpdates = self.batchUpdates;
    ...
    self.state = IGSTListBatchUpdateStateExecutingBatchUpdateBlock;
    ...
    for (IGSTListItemUpdateBlock itemUpdateBlock in batchUpdates.itemUpdateBlocks) {
        itemUpdateBlock();
    }
    ...
    self.state = IGSTListBatchUpdateStateExecutedBatchUpdateBlock;
    ...
    for (IGSTListUpdatingCompletion block in completionBlocks) {
        block(YES);
    }

    self.state = IGSTListBatchUpdateStateIdle;
}

static NSArray *objectsWithDuplicateIdentifiersRemoved(NSArray<id<IGSTListDiffable>> *objects) {
    if (objects == nil) {
        return nil;
    }
    ...
    for (id<IGSTListDiffable> object in objects) {
...
- (void)performBatchUpdatesWithCollectionView:(UICollectionView *)collectionView {
    IGAssertMainThread();
    IGAssert(self.state == IGSTListBatchUpdateStateIdle, @"Should not call batch updates when state isn't idle");
    ...
    const IGSTListExperiment experiments = self.experiments;

    IGSTListIndexSetResult *(^performDiff)() = ^{
        return IGSTListDiffExperiment(fromObjects, toObjects, IGSTListDiffEquality, experiments);
    };

    // block executed in the first param block of -[UICollectionView performBatchUpdates:completion:]
    void (^batchUpdatesBlock)(IGSTListIndexSetResult *result) = ^(IGSTListIndexSetResult *result){
        executeUpdateBlocks();
    ...
    void (^performUpdate)(IGSTListIndexSetResult *) = ^(IGSTListIndexSetResult *result){
        @try {
            [delegate listAdapterUpdater:self willPerformBatchUpdatesWithCollectionView:collectionView];
            if (result.changeCount > 100 && IGSTListExperimentEnabled(experiments, IGSTListExperimentReloadDataFallback)) {
                reloadDataFallback();
            } else if (animated) {
    ...
    if (IGSTListExperimentEnabled(experiments, IGSTListExperimentBackgroundDiffing)) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            IGSTListIndexSetResult *result = performDiff();
...
- (IGSTListBatchUpdateData *)flushCollectionView:(UICollectionView *)collectionView
                                withDiffResult:(IGSTListIndexSetResult *)diffResult
                                  batchUpdates:(IGSTListBatchUpdates *)batchUpdates
                                   fromObjects:(NSArray <id<IGSTListDiffable>> *)fromObjects {
    ...
    if (self.movesAsDeletesInserts) {
        for (IGSTListMoveIndex *move in moves) {
    ...
    NSMutableArray<IGSTListMoveIndexPath *> *itemMoves = batchUpdates.itemMoves;
    ...
    for (IGSTListReloadIndexPath *reload in batchUpdates.itemReloads) {
...
- (void)cleanStateAfterUpdates {
    self.batchUpdates = [IGSTListBatchUpdates new];
}
...
- (void)queueUpdateWithCollectionView:(UICollectionView *)collectionView {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.state != IGSTListBatchUpdateStateIdle
            || ![weakSelf hasChanges]) {
            return;
        }
...
#pragma mark - IGSTListUpdatingDelegate

static BOOL IGSTListIsEqual(const void *a, const void *b, NSUInteger (*size)(const void *item)) {
    const id<IGSTListDiffable> left = (__bridge id<IGSTListDiffable>)a;
    const id<IGSTListDiffable> right = (__bridge id<IGSTListDiffable>)b;
...
static NSUInteger IGSTListIdentifierHash(const void *item, NSUInteger (*size)(const void *item)) {
    return [[(__bridge id<IGSTListDiffable>)item diffIdentifier] hash];
}
...
- (NSPointerFunctions *)objectLookupPointerFunctions {
    ...
    functions.hashFunction = IGSTListIdentifierHash;
    functions.isEqualFunction = IGSTListIsEqual;
    return functions;
}
...
- (void)performUpdateWithCollectionView:(UICollectionView *)collectionView
    ...
    for (id obj in toObjects) {
        IGAssert([obj conformsToProtocol:@protocol(IGSTListDiffable)],
                 @"In order to use IGSTListAdapterUpdater, object %@ must conform to IGSTListDiffable", obj);
    ...
    IGSTListUpdatingCompletion localCompletion = completion;
    if (localCompletion) {
...
- (void)performUpdateWithCollectionView:(UICollectionView *)collectionView
    ...
    if (self.state == IGSTListBatchUpdateStateExecutingBatchUpdateBlock) {
        itemUpdates();
    } else {
...
- (void)moveItemInCollectionView:(UICollectionView *)collectionView
    ...
    if (self.state == IGSTListBatchUpdateStateExecutingBatchUpdateBlock) {
        IGSTListMoveIndexPath *move = [[IGSTListMoveIndexPath alloc] initWithFrom:fromIndexPath to:toIndexPath];
...
- (void)reloadItemInCollectionView:(UICollectionView *)collectionView
                     fromIndexPath:(NSIndexPath *)fromIndexPath
                       toIndexPath:(NSIndexPath *)toIndexPath {
    if (self.state == IGSTListBatchUpdateStateExecutingBatchUpdateBlock) {
        IGSTListReloadIndexPath *reload = [[IGSTListReloadIndexPath alloc] initWithFromIndexPath:fromIndexPath toIndexPath:toIndexPath];
...
- (void)reloadDataWithCollectionView:(UICollectionView *)collectionView
                   reloadUpdateBlock:(IGSTListReloadUpdateBlock)reloadUpdateBlock
                          completion:(nullable IGSTListUpdatingCompletion)completion {
    IGAssertMainThread();
    IGParameterAssert(collectionView != nil);
