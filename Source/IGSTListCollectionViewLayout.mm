...
#import "IGSTListCollectionViewLayout.h"
...
@interface IGSTListCollectionViewLayoutInvalidationContext : UICollectionViewLayoutInvalidationContext
@property (nonatomic, assign) BOOL igst_invalidateSupplementaryAttributes;
@property (nonatomic, assign) BOOL igst_invalidateAllAttributes;
@end

@implementation IGSTListCollectionViewLayoutInvalidationContext
@end

@interface IGSTListCollectionViewLayout ()
...
@implementation IGSTListCollectionViewLayout {
    std::vector<IGSTListSectionEntry> _sectionData;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    ...
    const IGSTListSectionEntry entry = _sectionData[section];
...
- (CGSize)collectionViewContentSize {
    ...
    const IGSTListSectionEntry section = _sectionData[sectionCount - 1];
    UICollectionView *collectionView = self.collectionView;
...
- (void)invalidateLayoutWithContext:(IGSTListCollectionViewLayoutInvalidationContext *)context {
    ...
    if (hasInvalidatedItemIndexPaths
        ...
        || context.igst_invalidateAllAttributes) {
        _cachedLayoutInvalid = YES;
    }

    if (context.igst_invalidateSupplementaryAttributes) {
...
+ (Class)invalidationContextClass {
    return [IGSTListCollectionViewLayoutInvalidationContext class];
}

- (UICollectionViewLayoutInvalidationContext *)invalidationContextForBoundsChange:(CGRect)newBounds {
    const CGRect oldBounds = self.collectionView.bounds;

    IGSTListCollectionViewLayoutInvalidationContext *context =
    (IGSTListCollectionViewLayoutInvalidationContext *)[super invalidationContextForBoundsChange:newBounds];
    context.igst_invalidateSupplementaryAttributes = YES;
    if (!CGSizeEqualToSize(oldBounds.size, newBounds.size)) {
        context.igst_invalidateAllAttributes = YES;
    }
    return context;
}
...
- (void)setStickyHeaderYOffset:(CGFloat)stickyHeaderYOffset {
    ...
    if (_stickyHeaderYOffset != stickyHeaderYOffset) {
        ...
        IGSTListCollectionViewLayoutInvalidationContext *invalidationContext = [IGSTListCollectionViewLayoutInvalidationContext new];
        invalidationContext.igst_invalidateSupplementaryAttributes = YES;
...
- (void)cacheLayout {
    ...
    auto sectionData = std::vector<IGSTListSectionEntry>(sectionCount);
        ...
        for (NSInteger item = 0; item < itemCount; item++) {
            ...
            const CGRect rawFrame = (self.scrollDirection == UICollectionViewScrollDirectionVertical) ?
                ...
                CGRectMake(itemCoordInScrollDirection + insets.left,
                           ...
                           size.width,
                           itemLengthInFixedDirection);
            const CGRect frame = IGSTListRectIntegralScaled(rawFrame);
...
- (NSRange)rangeOfSectionsInRect:(CGRect)rect {
    ...
    const NSInteger sectionCount = _sectionData.size();
    for (NSInteger section = 0; section < sectionCount; section++) {
        IGSTListSectionEntry entry = _sectionData[section];
