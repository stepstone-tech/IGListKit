...
/**
 Implement this protocol to provide data to an `IGSTListAdapter`.
 */
NS_SWIFT_NAME(ListAdapterDataSource)
@protocol IGSTListAdapterDataSource <NSObject>
...
/**
 ...
 Section controllers are initialized for all objects whenever the `IGSTListAdapter` is created, updated, or reloaded.
 Section controllers are reused when objects are moved or updated. Maintaining the `-[IGSTListDiffable diffIdentifier]`
 guarantees this.
 */
